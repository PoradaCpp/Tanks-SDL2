...
void ButtonNamed::setRenderDimensions()
{
    ...
    int nSrcRectHeight = nHeight / m_SourceRectArr.size();

    for( int i = 0, nArrSize = m_SourceRectArr.size(); i < nArrSize; ++i)
    {
        m_SourceRectArr.at( i ) = { 0, nSrcRectHeight * i, nWidht, nSrcRectHeight };
    }
    ...
    m_NameText3D.setRelativeDestination( m_RelativeDestTxtRect, m_ImageTexture.getBase() );
}
